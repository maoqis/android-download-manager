...
public class DownloadListActivity extends Activity {
	...
	public class MyReceiver extends BroadcastReceiver {
		...
		private void handleIntent(Intent intent) {
			...
			if (intent != null
				...
				case MyIntents.Types.COMPLETE:
					...
					if (!TextUtils.isEmpty(url)) {
						...
						if (!TextUtils.isEmpty(path)) {
							StorageUtils.installAPK(DownloadListActivity.this,
									url);
						}
					}
					break;
				...
				case MyIntents.Types.ERROR:
					url = intent.getStringExtra(MyIntents.URL);
					String errorCode = intent.getStringExtra(MyIntents.ERROR_CODE);;
		...
		private void handleError(String url, String errorCode) {
			// TODO Auto-generated method stub
				if (errorCode.equals(DownloadTask.ERROR_BLOCK_INTERNET)) {

				} else if (errorCode.equals(DownloadTask.ERROR_UNKOWN_HOST)) {
					showAlert("错误", "无法连接网络");
					View taskListItem = downloadList.findViewWithTag(url);
					ViewHolder viewHolder = new ViewHolder(taskListItem);
					viewHolder.onPause();
				} else if (errorCode.equals(DownloadTask.ERROR_FILE_EXIST)) {
					showAlert("", "文件已经存在，取消下载");
				} else if (errorCode.equals(DownloadTask.ERROR_SD_NO_MEMORY)) {
					showAlert("错误", "存储卡空间不足");
				} else if (errorCode.equals(DownloadTask.ERROR_UNKONW)) {

				} else if (errorCode.equals(DownloadTask.ERROR_TIME_OUT)) {
	...
	private void showAlert(String title, String msg) {
		...
		new AlertDialog.Builder(DownloadListActivity.this).setTitle(title)
				.setMessage(msg)

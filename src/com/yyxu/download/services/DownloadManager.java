...
public class DownloadManager extends Thread {
		...
		for (int i = 0; i < mDownloadingTasks.size(); i++) {
			...
			if (task != null && task.getUrl().equals(url)) {
				...
				if (mDownloadingTasks.contains(task)) {
					ConfigUtils.clearURL(mContext,
							mDownloadingTasks.indexOf(task));
					mDownloadingTasks.remove(task);
					broadcastDeleteTask(task);
				}
				return;
			}
		}
	...
	private synchronized void broadcastDeleteTask(DownloadTask task) {

		Intent nofityIntent = new Intent(
				"com.yyxu.download.activities.DownloadListActivity");
		nofityIntent.putExtra(MyIntents.TYPE, MyIntents.Types.DELETE);
		nofityIntent.putExtra(MyIntents.URL, task.getUrl());
		nofityIntent.putExtra(MyIntents.PATH, task.getFile().getAbsolutePath());
		mContext.sendBroadcast(nofityIntent);

	}

	private synchronized void broadcastCompleteTask(DownloadTask task) {
	...
	private synchronized void broadcastErrorTask(DownloadTask task,
		...
		errorIntent.putExtra(MyIntents.ERROR_INFO, errorInfo);
		errorIntent.putExtra(MyIntents.URL, task.getUrl());
		if (errorCode.equals(DownloadTask.ERROR_FILE_EXIST)) {
			errorIntent.putExtra(MyIntents.PATH, task.getFile()
					.getAbsolutePath());
		}
		...
		DownloadTaskListener taskListener = new DownloadTaskListener() {
			...
			public void errorDownload(DownloadTask task, DownloadThrowable error) {
				...
				if (error != null) {
					...
					if (errorCode != DownloadTask.ERROR_UNKOWN_HOST
							&& errorCode != DownloadTask.ERROR_BLOCK_INTERNET
...
//							}
					...
					broadcastErrorTask(task, errorCode, errorMsg);
				}
